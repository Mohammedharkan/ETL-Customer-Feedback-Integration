...
-- Insert XML staging data into final Reviews table without duplicates
...
SELECT
    ...
    comments,
    review_date
FROM external_reviews_staging
WHERE NOT EXISTS (
    SELECT 1
    FROM reviews r
    WHERE r.customer_id = external_reviews_staging.customer_id
      AND r.product_id = 999
      AND r.rating = external_reviews_staging.rating
      AND r.comments = external_reviews_staging.comments
      AND r.review_date = external_reviews_staging.review_date
);

-- Verify final Reviews table after XML insertion
...
-- Common complaints analysis by keyword and category

SELECT
    p.category,

    CASE
        WHEN LOWER(r.comments) LIKE '%late%' THEN 'Late delivery'
        WHEN LOWER(r.comments) LIKE '%damaged%' THEN 'Damaged product'
        WHEN LOWER(r.comments) LIKE '%defective%' THEN 'Defective product'
        ELSE 'Other'
...
WHERE LOWER(r.comments) LIKE '%late%'
   OR LOWER(r.comments) LIKE '%damaged%'
   OR LOWER(r.comments) LIKE '%defective%'

GROUP BY p.category, complaint_type
...
-- Customer sentiment classification based on ratings
...
SELECT
    p.category,
    ROUND(AVG(r.rating), 2) AS average_satisfaction_score,
    COUNT(r.review_id) AS total_reviews
FROM reviews r
JOIN products p
ON r.product_id = p.product_id
